...
fetch('../../datas/datas.csv') // Je récupère le fichier CSV
  ...
  .then(texte => { // "Quand c'est du texte, fais quelque chose avec, on a une sorte de fonction"
    ...
    const colonnes = ['Ranking_Saison','Nom', 'Prénom', 'Performance', 'Validation', 'Date']; // On crée une constante avec les noms des colonnes qu'on veut afficher
    ...
    const idxRanking_Saison = entetes.indexOf('Ranking_Saison');
    ...
    function afficherTableau(filtreEpreuve = '', filtreSaison = '', filtreSexe = '', filtreCategorie = '', filtreGroupe = '') {
      ...
      const lignesFiltrees = donnees.filter(ligne => {
        ...
        return matchEpreuve && matchSaison && matchSexe && matchCategorie && matchGroupe;
      });

      lignesFiltrees.sort((ligneA, ligneB) => {
        const rankA = Number(ligneA.split(';')[idxRanking_Saison]);
        const rankB = Number(ligneB.split(';')[idxRanking_Saison]);
        return rankA - rankB;
      });
    ...
    function lireFiltresEtAfficher() {
      ...
      afficherTableau(epreuve, saison, sexe, categorie, groupe);
    }
